...
namespace E_ShopManagementSystem
{
    public partial class resetPassword : Form
    {
        ...
        {
            InitializeComponent();
            this.FormClosing += new FormClosingEventHandler(resetPassword_FormClosing);
       ...
       string.IsNullOrWhiteSpace(usernamebox.Text) ||// pass field field
       ...
       DOByear.SelectedIndex == -1)  // Date of birth not fully selected
            ...
            string result = VerifyIdentity(phone, username, day, month, year);
            ...
            {
                PassReset resetForm = new PassReset (phone, username); // Create instance of Form2
        ...
        private string VerifyIdentity(string Phone, string username, int day, int month, int year)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                string query = "SELECT * FROM UserInfo WHERE (Username = @Username OR Email = @Email) AND Phone = @Phone ";
                ...
                if (reader.Read()) // User found
                {
                    ...
                    if ((username == storedUsername  && storedDob == userDob))
                    {
                        return "Verification successfull.";
                    }
                    else if  (username == storedEmail && storedDob == userDob)
                    {
                        return "Verification successfull.";
                    }
                    else
